        ...
        const register = async () => {
            try {
                ...
                const handle = await CapApp.addListener('backButton', async ({ canGoBack }) => {
                    ...
                    //      React component.
                    try {
                        const stores = await import('/@/renderer/store');
                        const fsState = stores.useFullScreenPlayerStore.getState();
                        ...
                        const appStore = await import('/@/renderer/store/app.store');
                        const app = appStore.useAppStore.getState();
                        if (app.commandPalette.opened) {
                            app.commandPalette.close();
                            return;
                        }
                    } catch {
                        // Store unavailable for any reason — fall through to
                        // history-based back below.
                    }
