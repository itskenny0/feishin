...
export const SharedFullscreenPlayerSettings = () => {
    ...
    /*
     * Controlled-popover state so we can force-close it when the fullscreen
     * visualizer opens. The popover lives in a Mantine portal — when its
     * trigger (the cog) is removed from the DOM (the mobile overlay unmounts
     * once visualizerExpanded flips true), the portalised popover isn't
     * cleaned up with it and stays open on top of the visualizer canvas.
     */
    ...
    return (
        <>
            <ActionIcon
                ...
                variant="subtle"
            />
            <Popover onChange={setConfigOpen} opened={configOpen} position="bottom-start">
                <Popover.Target>
                    <ActionIcon
                        icon="settings2"
                        iconProps={{ size: 'lg' }}
