...
describe('command-dispatcher coalescing', () => {
    ...
    it('collapses a burst of rapid setVolume calls into leading + trailing only', { timeout: 15_000 }, async () => {
        ...
        const { commandDispatcher } = await import(
            '/@/renderer/features/jellyfin-remote-target/controller/command-dispatcher'
        );
        const ctx = {
            server: {
                credential: 't',
     ...
     */
    it('does not block setVolume against a new sessionId while a slow call to a previous sessionId is in flight', { timeout: 15_000 }, async () => {
