...
// Re-exported from the dependency-free `grid-layout` helper so existing
// importers keep their import path. The skeleton imports these directly from
// `grid-layout` to avoid pulling the grid engine into its chunk.
export { getDynamicItemsPerRow, GRID_CARD_CORNER_RADIUS_VALUE };
export type { GridCardCornerRadius };
...
const ListComponent = memo((props: ListChildComponentProps<GridItemProps>) => {
    ...
    // The per-cell wrapper style + className are identical for every cell in
    // every row of the grid (they only depend on the corner-radius / column
    // count / gap), so hoist them to a memo keyed on those values instead of
    // minting a fresh object + running clsx for each of the (rows × columns)
    // cells on every ListComponent render.
    const cellStyle = useMemo<CSSProperties>(
        ...
        [cornerRadiusValue, columns],
    );
    const cellClassName = useMemo(() => clsx(styles.itemRow, styles[`gap-${gap}`]), [gap]);
    ...
    for (let i = startIndex; i <= stopIndex + columnCountToAdd; i += 1) {
        if (i < itemCount) {
            ...
            items.push(
                <div className={cellClassName} key={`card-${i}-${index}`} style={cellStyle}>
                    <ItemCard
                        controls={controls}
                        data={item}
                        enableDrag={enableDrag}
