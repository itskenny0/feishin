...
async function buildSeeded() {
    ...
    const auth =
        'MediaBrowser Client="Feishin", Device="strict", DeviceId="' +
        deviceId +
        '", Version="1.11.0"';
...
async function main() {
    ...
    try {
        ...
        const warmSession = [];
        warmSession.push(
            await measureStrict(page, { name: 'Albums', path: '#/library/albums' }, albumName),
        );
        warmSession.push(
            await measureStrict(page, { name: 'Artists', path: '#/library/album-artists' }),
        );
            ...
            return new Promise((resolve) => {
                ...
                open.onsuccess = () => {
                    ...
                    req.onsuccess = () => {
                        ...
                        db.close();
                        resolve({ id: r?.Id, name: r?.Payload?.name });
                    };
                    req.onerror = () => {
                        db.close();
                        resolve(null);
        ...
        if (playlistId?.id) {
            const playlistDetail = await measureStrict(
                page,
                {
                    name: 'PlaylistDetail',
        ...
        const filterLogs = consoleLog
            .slice(baselineLogCount)
            .filter((m) => /filter: albums|filter: artists/.test(m.text));
        ...
        results.cacheLogs = consoleLog
            .filter((m) => /\[cache\]/.test(m.text))
            .slice(-50)
            .map((m) => m.text);
        results.errors = consoleLog
...
async function measureStrict(page, surface, knownText) {
    ...
    const onReq = (req) => {
        const u = req.url();
        if (
            u.includes('demo.jellyfin.org') &&
            (u.includes('/Items') || u.includes('/Users/') || u.includes('/Playlists'))
        ) {
            reqs.push(u);
        }
    };
        ...
        return new Promise((resolve) => {
            ...
            open.onsuccess = () => {
                ...
                req.onsuccess = () => {
                    const r = req.result?.[0];
                    db.close();
                    resolve(r?.Payload?.name || r?.SortName || null);
                };
                req.onerror = () => {
                    db.close();
                    resolve(null);
