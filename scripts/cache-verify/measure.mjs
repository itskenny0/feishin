...
async function dismissOptInEnable(page) {
    ...
    const toastClose = page
        .locator('button[aria-label="Close notification" i], button[aria-label*="close" i]')
        .first();
...
async function loginViaForm(page) {
    ...
    if (
        !page.url().includes('action-required') &&
        !(await page
            .getByText(/server required/i)
            .first()
            .isVisible()
            .catch(() => false))
    ) {
    ...
    try {
        await page.waitForFunction(() => !location.hash.includes('action-required'), null, {
            timeout: 30_000,
        });
    } catch {
...
const sweepWatcher = { hydrateCompleteAt: 0, lastEventAt: 0, totalEvents: 0 };
...
async function main() {
    ...
    const measureColdSurface = async (surface, opts = {}) => {
        ...
        cpage.on('console', (m) =>
            allConsole.push({ phase: 'cold', text: m.text(), type: m.type() }),
        );
        ...
        log(
            '  cold',
            r.surface,
            'visible=' + r.visibleMs + 'ms',
            'idle=' + r.idleMs + 'ms',
            r.apiRequestCount + 'api',
        );
    ...
    page.on('console', (m) => allConsole.push({ phase: 'warm', text: m.text(), type: m.type() }));
    page.on('pageerror', (e) =>
        allConsole.push({ phase: 'warm', text: e.message, type: 'pageerror' }),
    );
    ...
    try {
        ...
        log(
            'LS auth currentServer:',
            lsDump1.auth?.state?.currentServer?.id,
            'userId:',
            lsDump1.auth?.state?.currentServer?.userId,
        );
        ...
        log(
            'After opt-in: settings localCache:',
            JSON.stringify(lsDump2.settings?.state?.localCache),
        );
        ...
        if (ids.artistId) {
            detailSurfaces.push({
                name: 'ArtistDetail',
                path: '#/library/album-artists/' + ids.artistId,
            });
        }
        if (ids.playlistId) {
            detailSurfaces.push({
                name: 'PlaylistDetail',
                path: '#/playlists/' + ids.playlistId + '/songs',
            });
        }
...
async function measureSearchAsYouType(page) {
    ...
    for (let i = 0; i < keystrokes.length; i++) {
        ...
        await page.goto(BASE + '/#/search/album?query=' + encodeURIComponent(q), {
        ...
        samples.push({ ms: Math.round(t1 - t0), q });
    }
    return samples;
}
...
async function measureSurface(page, surface) {
    ...
    let visibleAt = null;
    const visibleOk = await page
        .waitForFunction(
            () => {
                // Any img with a src pointing at jellyfin = data has loaded.
                const imgs = document.querySelectorAll(
                    'main img[src*="jellyfin"], main img[src^="blob:"], main img[src^="data:"]',
                );
    ...
    await page.evaluate(async () => {
        ...
        try {
            for (const k of Object.keys(sessionStorage)) {
                if (k.startsWith('feishin:hydration-banner-dismissed'))
            ...
            try {
                const text = await page.evaluate(() => {
                    const headers = document.querySelectorAll(
                        'header, [class*="LibraryHeaderBar"], [class*="library-header"]',
                    );
                    ...
                    const fetchOne = (store) =>
                        new Promise((r) => {
                            ...
                            const req = db
                                ...
                                .getAll(null, 1);
                            req.onsuccess = () =>
                                r(req.result && req.result[0] ? req.result[0] : null);
                            req.onerror = () => r(null);
                        });
                    Promise.all([
                        fetchOne('albums'),
...
async function waitForSweepIdle(page, timeoutMs = 90_000) {
    ...
    while (Date.now() - t0 < timeoutMs) {
        if (sweepWatcher.hydrateCompleteAt > t0) {
            return {
                complete: true,
                sweepEvents: sweepWatcher.totalEvents,
                wallMs: Date.now() - t0,
            };
        }
